...
function showDemo151(id) {
    ...
    const URL = 'https://garevna-rest-api.glitch.me/';
    ...
    const showUsers = response => {
        if(demo.getElementsByClassName('user-container')[0]) {
            demo.getElementsByClassName('user-container')[0].innerHTML = ''
        }

        const usersContainer = demo.getElementsByClassName('user-container')[0] ?
            demo.getElementsByClassName('user-container')[0] :
            createEl(demo,'div', {className: 'user-container'});

        createEl(usersContainer, 'div', {
        ...
        for(let userData in response) {
            let userEl = createEl(usersContainer, 'div', {className: 'user-row'});
            renderUser.call({
                name: response[userData].name,
                age: response[userData].age,
                speciality: response[userData].speciality,
                id: userData
            }, userEl)
        }
        const addUsers = createEl(usersContainer, 'div', {
            className: 'user-row',
            innerHTML: `<form>
                                <div><input name="name" placeholder="Name"/></div>
                                <div><input name="age" placeholder="Age"/></div>
                                <div><input name="speciality" placeholder="Speciality"/></div>
                            </form>`
        });

        createEl(createEl(addUsers), 'button', {
            innerHTML: 'ADD',
            onclick: addUser.bind(null, addUsers)
        });
    ...
    async function addUser(container, event) {
        let data = new FormData(container.getElementsByTagName('FORM')[0]);
        let response = await fetch ( `${URL}user/${Math.floor(Math.random()*1000000)}`, {
            method: "POST",
            headers: {
        ...
        response.status === 200 ? renderUsers() : reject(new Error('Something went wrong'))
    }

    async function saveUser(container, event) {
        let data = new FormData(container.getElementsByTagName('FORM')[0]);
        let response = await fetch ( `${URL}user/${this.id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json"
            },
    ...
    async function deleteUser(container, event) {
        let response = await fetch ( `${URL}user/${this.id}`, {
            method: "DELETE"
        } );
        response.status === 200 ? renderUsers() : reject(new Error('Something went wrong'))
    ...
    function editUser(userEl, event) {
        userEl.innerHTML = `<form>
                                <div><input name="name" value="${this.name}"/></div>
                                <div><input name="age" value="${this.age}"/></div>
                                <div><input name="speciality" value="${this.speciality}"/></div>
                            </form>`

        createEl(createEl(userEl), 'button', {
            innerHTML: 'SAVE',
            onclick: saveUser.bind(this, userEl)
        });

        createEl(createEl(userEl), 'button', {
            innerHTML: 'CANCEL',
            onclick: renderUser.bind(this, userEl)
        });
    }

    function renderUser(userEl) {
        let { name = 'no info', age = 'no info', speciality = 'no info' } = this;
        userEl.innerHTML = `<div>${name}</div><div>${age}</div><div>${speciality}</div>`

        createEl(createEl(userEl), 'button', {
            innerHTML: 'EDIT',
            onclick: editUser.bind(this, userEl)
        });

        createEl(createEl(userEl), 'button', {
            innerHTML: 'DELETE',
            onclick: deleteUser.bind(this, userEl)
        });
    }

    function renderUsers() {
        getUsers ().then ( showUsers, reject )
    }
    renderUsers()
}

showDemo151('demo_15_1');
codePreview('demo_15_1', showDemo151);

function showDemo153(id) {
    let [ demo, submitBtn, userLoginInput, userPass, userPassConf, photo ] =
        [ 'demo_15_3', 'submit-15-3', 'login-15-3', 'pass-15-3', 'pass-conf-15-3', 'file-input-15-3']
    ...
    function validateUserCred() {
        for(let key in userCred) {
            if(!userCred[key]) return true
        }
        return false
    }
    
    function showPageForUsers(user) {
        demo.innerHTML = `Hi ${user}, you are logged in!`
    }
    ...
    function getInput ( users ) {
        ...
        userLoginInput.oninput = event => {
            let test = logins.filter (
                login => login === event.target.value
            ).length > 0;

            event.target.style.borderColor = test ? "red" : "";
            event.target.title = test ? "User is already exist" : "";
            userCred.login = test ? null : event.target.value;
            submitBtn.disabled = validateUserCred();
        };
        confirmPass();
        checkPhoto();
        submit();
    ...
    function confirmPass() {
        userPass.oninput = event => {
            let test = event.target.value.length < 6;
            event.target.style.borderColor = test ? "red" : "";
            event.target.title = test ? "Password should contain at least 6 characters" : "";
            userCred.password = test ? null : event.target.value;
            submitBtn.disabled = validateUserCred();
        };
        userPassConf.oninput = event => {
            let test = event.target.value !== userCred.password;
            event.target.style.borderColor = test ? "red" : "";
            event.target.title = test ? "Should be same as password" : "";
            userCred.passwordConf = !test;
            submitBtn.disabled = validateUserCred();
        };
    }
    ...
    function showPhotoError(target) {
        userCred.photo = null;
        let label = target.parentElement.getElementsByTagName('label')[0];
    ...
    function checkPhoto() {
        photo.onchange = function(event) {
            event.target.files[0].type.indexOf('image') === 0 && event.target.files[0].size <= 100000 ?
                showPhoto(event.target.files[0]).then(img => {
                    userCred.photo = img;
                    let curImg = event.target.parentElement.getElementsByTagName('img')[0] ?
                        event.target.parentElement.getElementsByTagName('img')[0] :
                    ...
                    if(label.classList.contains('error')) {
                        ...
                        event.target.parentElement.getElementsByTagName('span')[0].remove();
                    }
                    submitBtn.disabled = validateUserCred();
                }) :
                showPhotoError(event.target);
        };
    }
    ...
    function checkUser(users) {
        ...
        let logins = Object.keys ( users );
        let loggedUser = logins.find ( login => login === userLogin && users[login].hash === userHash );

        loggedUser ? showPageForUsers(loggedUser) : getInput ( users )
    }
    ...
    async function sendCred () {
        ...
        document.cookie = `login=${userCred.login}; expires=${expDate}`;
        document.cookie = `hash=${Sha256.hash ( userCred.password )}; expires=${expDate}`;

        return await fetch ( `${URL}user/${userCred.login}`, {
            method: "post",
            headers: {
                "Content-Type": "application/json"
            },
            body: JSON.stringify({
                name: userCred.login,
                hash: Sha256.hash ( userCred.password ),
                photo: userCred.photo
            })
        } )
    }
        ...
        submitBtn.onclick = function (event) {
            ...
            sendCred ().then(
                respose => respose.json()
                    .then(data => {showPageForUsers(data.name)}),
...
showDemo153('demo_15_3');
codePreview('demo_15_3', showDemo153);

function showDemo161(id) {
    class ClimaticZones {
        constructor(region) {
            this.region = region
        };
    ...
    let region1 = new ClimaticZones('Экваториальные области Африки')
    console.log(region1.region,' :', region1.getClimate());
}
